...
extern "C" int init(int argc, const char *argv[]) {
  ...
  *verbose = OFF;
  ...
  if(Statesptr->ChannelPresent==OFF) ChannelSegmentsVecPtr->resize(1); // temp fix to prevent visual studio debuger exiting on the next line (JCN); here some ISSUES with BMI
